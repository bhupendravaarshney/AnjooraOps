...
export async function POST(request) {
  ...
  if (!expected || !secureEqual(auth, expected)) return new Response('Unauthorized', { status: 401 });
  ...
  for (const candidate of rows) {
    ...
    const result = await withTransaction(async (db) => {
      ...
      const message = await queueWhatsAppTemplate({
      ...
      await writeAudit(db, {
        ...
        payload: { event_key: eventKey, message_id: message.messageId },
      });
      return message;
    });
  ...
  return Response.json({ ok: true, due: rows.length, queued, skipped, templateConfigured: Boolean(templateName) });
}
